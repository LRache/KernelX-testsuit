...
# BusyBox ash compatibility edits: safer timeout number parsing.
...
_tst_multiply_timeout()
{
	...
	if ! tst_is_int "$LTP_TIMEOUT_MUL"; then
		...
		[ -z "$LTP_TIMEOUT_MUL" ] && LTP_TIMEOUT_MUL=0
		LTP_TIMEOUT_MUL=$((LTP_TIMEOUT_MUL + 1))
	...
	if ! tst_is_int "$timeout"; then
		tst_brk TBROK "timeout must be integer >= 1 ($timeout)"
	fi

	[ "$timeout" -ge 1 ] || tst_brk TBROK "timeout need to be >= 1 ($timeout)"

	timeout=$((timeout * LTP_TIMEOUT_MUL))
	eval "$1=\"$timeout\""
	return 0
}
